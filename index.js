            ...
            if(answer === '1'){
                ...
                readline.question(`Enter a Date ex: 01 Jan 2018: \n`,(date) => {
                    ...
                    stream.on('data',(data)=>{
                        ...
                        const url=`https://min-api.cryptocompare.com/data/v2/histoday?fsym=BTC&tsym=USD&limit=1&toTs=${parseDate/1000}&api_key=${cryptoKey}`;
                        request({url:url},(error,res,body)=>{
                        if(!error&&res.statusCode==200){
                            const parseData=JSON.parse(body);
                            ...
                            console.log(`Total value of all tokens on this date: ${date} is $:${newTotal.toLocaleString()}`) 
                        }
                        })
                    });
                    readline.close();
                ...
                readline.question(`Enter a Date ex: 01 Jan 2015:\n`,( date => {
                    ...
                    readline.question(`Enter a Token:\n`,(input=>{
                        ...
                        stream.on('data',(data)=>{
                            ...
                            const url=`https://min-api.cryptocompare.com/data/v2/histoday?fsym=${token}&tsym=USD&limit=1&toTs=${parseDate/1000}&api_key=${cryptoKey}`;
                            request({url:url},(error,res,body)=>{
                                if(!error&&res.statusCode==200){
                                    const parseData=JSON.parse(body);
                                ...
                                console.log('at ' + date + token +' value was:' + parseData['Data']['Data'][1]['close']);
                                    console.log('Total Amount of '+ input +': $'+btc.toLocaleString()+` for ${date}`);
